...
请求日志详情里的「智能应答判定」块，以及 MCP `lum_llm_mock_requests` 返回的 `smartMeta`：
...
> 要看完整报文走平台「请求日志」页或 MCP `lum_llm_mock_requests`，那两条路都要登录/授权。
...
## 向量 mock 的 SIM 指令（语义缓存测试专用）

跟上面的 `MODE:`/`SAY:` 是同一路子——**把行为写进请求、而不是配在页面上**——但作用在
**embeddings** 路由（`/v1/embeddings`），供网关的「系统设置 → 缓存（语义缓存）」联调。

**为什么要有它。** 不带指令时，mock 按输入文本**确定性**生成向量，相似度实质是**字面重合度**，
不是语义。这有两个测不到的窟窿：

- **同义句分数偏低**：`今天几号` / `今天是几号` 只有 ~0.69，够不到默认 0.95 阈值 ——
  于是「缓存命中」这条正路根本触发不了，测不了。
- **反义句分数偏高**：`删除用户` / `不要删除用户` 字全重合，分数很高，会**误命中** ——
  而这正是语义缓存最危险的错，旧 mock 恰恰暴露不出来。

**指令写法**（写在发给 embeddings mock 的输入文本里，`fixed` 固定向量优先级更高、会盖过它）：

| 写法 | 效果 |
|---|---|
| `SIM:q1` | 归到「q1」这组，向量 = 该组锚点，和同组其它文本余弦 **1.0** |
| `SIM:q1@0.97` | 和「q1」组锚点余弦 **精确 0.97**（`@` 后取 0~1，用来卡阈值上下沿） |
| 不同键（`SIM:a` vs `SIM:b`） | 两组锚点近正交，余弦 **≈0**，必然「未命中」 |

配方：同义句都标 `SIM:q1` → 必命中；边界句标 `SIM:q1@0.94` → 默认阈值下必未命中；
反义句各标一个键 → 字面再像也 ≈0，测得出「反义误命中」。数学是 Gram-Schmidt 正交构造，
`b = score·锚 + √(1-score²)·正交分量`，余弦被钉到小数点后四位（实测 0.97→+0.9700）。
...
| `backend/app/services/llm_mock_engine.py` `parse_sim_directive` / `controlled_vector` | 向量 SIM 指令：解析 + 按余弦精确构造向量 |
