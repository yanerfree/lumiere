...
⚠ **2026-09-04 这条推论被 §12 改了**：理由（「它不知道自己造了什么、也清理不掉」）
成立，但结论下窄了。**自己亲手造出来、名字带自己前缀的那一条数据是有向的** ——
在它身上点编辑/删除，我们知道点的是什么，也知道怎么清。改后的判据见 §12.2；
「不认识的控件不点」这条对**别人的数据**照旧成立。
...
⚠ **这一节的做法 2026-09-04 已被替换，别照着做。** 原计划是「按域手写 8–12 条
Playwright 脚本」；用户当场否掉（原话：「做通用型的，不是一个一个写代码，
这个域能用，下一个域就都能用，换一个项目也能用的」）。
实际落地的是通用爬取（§11），以及**它还差的那一半**——真的走一遍
新建→编辑→删除（§12）。**§12 是这一批的现行需求。**
