...
### QA 仓侧（§2/§3）今天没跑：跑不了，不是忘了

`git fetch` 那个 bare 缓存（`backend/.qa-repos/1a1fb724-….git`，UAG 项目那把）
**在 worktree 里做不到** —— 会话被隔离在 `.claude/worktrees/` 下，任何指向别的仓库
的 git 操作（`cd` 过去、`--git-dir=` 重定向）都会被挡掉。这不是权限问题也不是网络问题，
换个地方跑就行：**在主 checkout 里跑**，命令原样照 §2 / §3 那两个代码块，别改判据。

⚠ 补跑的时候有两条别忘：
1. **先 `git fetch` 再统计**，否则统计的是缓存里那份旧快照 —— 而「QA 仓一直在动」
   恰好是 §7 第 1 条要防的事，拿旧快照重跑等于**假装重跑过了**。
2. 只用 `show`/`ls-tree`/`rev-parse`/`log`/`grep`/`blame`（CLAUDE.md 那条硬规则）。
   `fetch` 只往本地缓存写，不碰对方仓库，所以它不在禁的那一类里。

**这三个数字要是变了，影响的是 §6 批 2 要写几条脚本，不影响批 1 能不能开工** ——
批 1 是「一趟只读跑通全链」，跑通不依赖端点总数。
