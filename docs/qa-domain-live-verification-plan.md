...
| 4 | **每一步读业务逻辑**：规则（必填 / 提示原文）· 状态 · 现在还能点什么 · 详情页多出了什么区块。业务动作不只增删改查，动作面两边各出一半枚举 | ⬜ 需求见 §14 |
| 5 | **广度必须满、深度可以打折**：页面上有的功能一个都不许没看到；没走的显式记成没走，`unreached` 和 `seen_not_run` 不许合并 | ⬜ 需求见 §15 |

**剩下 10~15% 不追**：尾巴上是一次性的 `POST /admin-users`、
`DELETE /audit/logs/{id}` 这类，报告里标「未覆盖」比硬凑一个假覆盖诚实。

⚠ **这句只管深度。** 广度（页面上有哪些功能）**必须满** —— 那个接口对应的按钮
只要在页面上，照样必须被「看到」并记下来。判据见 §15。
...
### 13.2b ⚠ 「点了哪个按钮 → 发出哪条请求」这张表**现在不存在**，得先做出来

这是 2026-09-04 查库查出来的，别按"现成的"来排期：

- `qa_page_survey_items.endpoints` 这一列**建了，但一行都没写过** ——
  最近一趟 1361 个控件行里，记着「点它会发出什么请求」的是 **0 行**。
- 现在只有**页面级**的 P 边（打开这一页浏览器发了什么，668 条），
  归页规则在 `qa_page_traffic`；控件级只记到「点了有没有反应」
  （跳走 / 弹层 / 没动静），**没记发出了什么**。
- `qa_page_survey_crawl.py` 里那段注释说得很清楚，而且当时是对的：
  「这一趟一个控件都没点过，写进去等于凭空造一条 `observed` 的控件→端点边」。
  批 2 之后已经真点了（23 次），**所以这条边现在该补，那句注释也该改**。

补的办法和页面级是同一招 —— **按时间窗归属**：点下去之前打个标记，
之后一小段时间里新出现的请求就算这次点击发出的（页面级用的是导航时窗，
控件级换成点击时窗）。这不是新机器，但**是活儿**，§12 要一起做。

没有这张表，§13.2 的骨架就落不到按钮上：知道「他测的第二步是提交」，
但不知道页面上哪个按钮会发出那条请求 —— 只能靠按钮名字去猜，
**那就退回 §4 否掉的那条路了**。

拿到这张表之后，两边一拼就知道**该按什么顺序点哪几个按钮** ——
全程没有一个产品名词，换域换产品照样成立。
...
## §14 需求补充（2026-09-04 下午，第三轮）：业务逻辑要**读出来**，而且业务**不只是增删改查**

用户当场定，原话两句：

> 「探索测试，就是探索这个页面设计的所有功能，当然不是只看反应啊，
> 你要看业务逻辑呀，不然怎么往后走呢」
>
> 「业务不只是增删改查」

两句各否掉一件事：第一句否掉「点完只记有没有反应」，第二句否掉「一条 建→改→删 就算探完」。

### 14.1 每点一步，要落的是四样，不是「有没有反应」

批 2 的账本只记到三档（跳走 / 弹层 / 没动静）。那三档是**能不能继续往下走**的判据，
**不是业务**。每一步真正要落的是这四样 —— 全部不认识产品名词也能落：

| 读什么 | 从页面哪儿读 | 它告诉我们什么 |
|---|---|---|
| **规则** | 表单里哪些字段带必填标记；填错时弹出来的提示**原文** | 这个功能的约束（必填 / 格式 / 唯一 / 上限） |
| **状态** | 我们那一行的状态列文本 | 这条业务现在停在哪一格（判法见 §13.4） |
| **动作面** | 我们那一行 / 详情页上**现在**还能点什么、哪些按钮变灰了 | **下一步该干什么，是页面自己给出的** |
| **结构** | 详情页里多出来的区块（审批记录、操作日志、关联列表） | 后面还有谁要接手、这个对象跟谁有关系 |

⚠ 四样里最要紧的是**动作面**：有了它，链路能**自己往前推** ——
「状态文本变了 + 冒出了新按钮」就是往前走了一格，
**不需要拿按钮名字去猜下一步**（那正是 §4 否掉的那条路）。

⚠ 「填错时的提示」要落**原文**，别落成 pass/fail。提示原文**就是规则本身**，
压成一个位之后，下一趟没人知道当初触发的是哪条约束。
（和采集侧同一条口径 —— `qa_page_survey_crawl._COLLECT_JS` 里那条 ⚠：
字段文案**绝不能取 `value`**，那是填进去的东西。**label 和提示文案要落，`value` 永不落。**）

### 14.2 业务不只是增删改查：**动作面**怎么枚举出来

「建 → 改 → 删」只是最省事的骨架。真实的业务动作还有：
提交、审批、驳回、启用、停用、发布、下线、复制、导入、导出、批量、
绑定 / 解绑、重试、重置、穿透跳转……

**一条都不许靠"列个清单去认"** —— 清单必然漏，而且换个产品就烂掉。
两边各出一半，两半都不含产品名词：

- **页面这一半 —— 把「能点的地方」找齐，不止表格上方那排按钮：**
  行内动作列、行末「更多」下拉、**勾选一行之后才冒出来的批量条**、
  详情页的每个页签、页签里的按钮、弹层里的次级按钮、状态列上的开关。
  这些现有的控件枚举本来就抓得到，**欠的是按「属于哪一行 / 哪一层」归类**，不是欠抓取。

- **脚本这一半 —— 动作接口的最后一段路径就是业务动作名：**
  `POST /x/{id}/submit` 的 `submit`，以及 `/approve`、`/enable`、`/publish`、`/import`。
  Q 边解析器已经在抽调用点，**取末段、末段不是 id 就当动作词**即可。
  于是「这个域有哪些非增删改查的动作」是**从对方脚本里数出来的**，不是我们编的。

两边一拼 —— **连接键是端点，不是按钮名字**（`qa_domain_map.pair_actions`）。
拿名字连要一张「submit ↔ 提交」的对照表，每个产品、每种语言各一张；
而漏一条的表现是**凭空多报一条缺口**（页面上明明有那个按钮），不是少报一条。
所以两边先各归成同一样东西：**端点末段那个动作词**（判法同上一条）。

页面那一半的端点从哪来：控件级 P 边（「点这个按钮的时候发了哪几条请求」）
按 **anchor** 接到功能地图的动作行上（`attach_control_endpoints`）。
两个反直觉的地方：

- **`page_path` 不进连接键。** 功能地图是**这个域**的地图，同一个「删除」在
  列表页和详情页上是同一个动作词；掺进页面路径只会让「详情页那个删除」
  凭空多出一条缺口。
- **「没点过」和「点了没发请求」都一个字不写。** `None` = 那一行没点过，
  写成空列表等于替它宣布「点了没发请求」；`[]` = 点过、确实一条没发 ——
  那是 G4 那一列的料，写进来会让这个按钮被当成「连上了、只是没有端点」。

拼出两类发现：

- 脚本打过这个动作端点，**页面上没有哪个按钮发过它** → 一条发现（他可能测的是
  页面走不到的路，也可能是我们没走到那一层 —— **只是发现，不是结论**，见 §13.3）。
- 页面上这个动作发过的端点，**脚本一次都没打过** → 一条发现（就是 G3 那一类，但这次带动作名）。

⚠ **两个清单都空不等于「对齐了」。** 控件级那一列还没落下来时（`endpoints` 全是
`NULL`）这两个清单**恒为空**，读起来像「页面和脚本完全一致」—— 所以另出一个
`paired` 位，两边有一边是空的就在页面上直接写「一条都没连上」。

### 14.3 状态机也是**数出来**的，不是猜的

同一张列表里**所有行的状态文本去重** = 这个对象一共有几种状态。
我们那一条数据走过的状态序列 = 走通了的路径；**没出现过的那些 = 没走到的分支**。
这条判据同样不认识那些词的含义，换域换产品照样成立。

### 14.4 走不动的时候：**提示就是规则**

点下去没成，页面给的三类回答都要**原样落**，它们是业务逻辑的一部分：

- 「XX 不能为空 / 格式不对 / 已存在」→ 落成这个功能的**约束**。
- 「无权限 / 403」→ 落成「这一步归别人」（接 §13.5，403 是路标）。
- 「当前状态不允许此操作」→ 落成状态机的**一条边**（这个状态下点不动这个动作）。

**三类都不算失败。** 只有「点了、没反应、也没给提示」才是断点（G4）。

### 14.5 产出：从「一条链」升级成「这个域的功能地图」

§13.6 那张业务链路表保留，但不够 —— 它只记了主链。这一批要交的是两份：

1. **功能地图**：每个动作一行 ——
   点哪儿 / 属于哪一行或哪一层（行内 · 批量 · 详情页某页签）/ 谁能点 /
   点完状态变成什么 / 没走通时的提示原文。
2. **状态清单**：这个对象有哪几种状态（列表里数出来的）、我们走通了哪几条边。

对账口径不变（§13.3）：**脚本只许提出假设，结论只由真点下去之后页面发出了什么决定。**

---

## §15 需求补充（2026-09-04 下午，第四轮）：**业务要打通，功能要看全** —— 深度可以打折，广度不许

用户当场定，原话：

> 「你要把业务打通，知道有哪些功能，可以不全部测试到位，
> 但是必须把页面上所有的功能都看到，不然就遗漏了」

### 15.1 广度和深度是**两个数**，别加权成一个「覆盖率」

- **广度（看到了没有）= 必须满。** 页面上有的功能，一个都不许没看到。
  「看到」的定义是三件事齐：枚举到了这个可点的位置 + 知道它属于哪一行 / 哪一层 +
  知道谁能点。**不要求点透。**
- **深度（走通了没有）= 允许不满。** 主链至少走通一条到底；支线可以只"看到"不"走"。
  没走的**必须显式记成"没走"** —— 省略掉之后，它和"走过、没问题"在报告里长得一模一样。

两个数**分两列出**，不许加权：加权之后「看全了但只走通一条」和
「只看了一半但都走通了」会拿到同一个分，而这两种情况欠的账完全不同。

### 15.2 「看全」依赖「打通」—— 这就是用户那句话里的因果

页面上有一批功能**不把业务往前走就根本不出现**：

- 不建数据 → 进不去详情页 → 详情页那几个页签、页签里的按钮，一个都看不到。
- 不勾选行 → 批量操作条不出现。
- 状态没到「待审」 → 「通过 / 驳回」不出现。
- 没有关联数据 → 「解绑」不出现。

所以顺序是**先打通、再清点**，不能反过来。批 1 那种「不建数据只逛」的爬法
**结构上就看不到这几层** —— 它数出来的功能数天然偏小，
而偏小的那部分**正好是业务最重的那部分**。

### 15.3 「没看到」必须留痕，还要写清**为什么没看到**

这是本文档一贯那条（缺信号不是一个结论）在功能面上的落地。三种「没看到」要分开记：

| 记成什么 | 什么情况 | 算不算我们的欠账 |
|---|---|---|
| `unreached` | 这一层要某个前置才出现，而前置没做到（没建数据 / 状态没到 / 没勾选） | **算** —— 把前置补上就能看到，进广度缺口 |
| `blocked` | 前置做到了，但手上所有角色都点不动（403 / 看不见） | 不算 —— 记「这一步归我们够不到的角色」 |
| `seen_not_run` | 看到了，**故意**没点（危险动作 / 深度打折） | 不算 —— 广度已经满了 |

⚠ **千万别把 `unreached` 和 `seen_not_run` 合成一个「未测」**：
前者是遗漏，后者是取舍，混在一起之后遗漏就再也报不出来了。

### 15.4 §6 那句「剩下 10~15% 不追」只管**深度**

那句说的是**接口层的深度**取舍（尾巴上一次性的 `POST /admin-users` 之类，
标「未覆盖」比硬凑一个假覆盖诚实）。它**不适用于广度**：
那个接口对应的按钮只要在页面上，照样必须被"看到"、必须记下来。

---

## §16 §12~§15 落地在哪（2026-09-04 已实现）

| 文件 | 干什么 | 对应哪一节 |
|---|---|---|
| `services/qa_business_actions.py` | 端点末段 → 业务动作名（末段是 id 就往前退一段）；`readable_paths=None` **不许变空集合** | §12 · §14.2 |
| `services/qa_page_traffic.py` | 两本时窗账：`bucket_entries` 按导航时窗归页（P 边）、`bucket_clicks` 按**点击时窗**归控件 | §13 |
| `services/qa_directed_chain.py` | 有向链路：建自己前缀的数据 → 走完 → 自带清理；断点 / 事实 / 残留 / 声明 | §13.6 · §14 |
| `services/qa_domain_map.py` | 功能地图（动作面 + 归类 + 谁能点 + 状态边）、状态清单、提示分类、广度/深度两个数、两边一拼 | §14 · §15 |
| `engine/surveys/qa_page_survey_crawl.py` | 真去点：有向链路在浏览器里怎么走、每步读那四样 | §13 · §14 |
| `services/qa_script_endpoints.py` | Q 边补动作词 + **文件内调用顺序** | §12 |
| `frontend/src/pages/qa/QaCatalog.jsx` | 「业务链路（有向）」「我们留下的残留」「广度 / 深度」「功能地图 + 状态清单」「动作面 × 脚本」 | §13.6 · §14.5 · §15 |
