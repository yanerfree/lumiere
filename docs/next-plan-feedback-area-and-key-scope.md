...
**状态：两条都已落地**（2026-09-03，分支 `worktree-wt-area-and-keyscope`）。
**落地和方案的差异、以及重跑出来的前提数字，全在文末 §3「落地记录」** —— 只想知道
「现在是什么样」的看那一节；下面 §1/§2 是**当初的分析和取舍**（2026-09-02 写的），
保留是因为里面的**判据**和两节「别这么写」还在生效，改这块之前仍要整篇读完。

⚠ 文档里的数字都是 2026-09-02 在 `lumiere` 库和 `TOOL_CATALOG` 上真跑出来的。
**再动这两块之前还要重跑一遍**（怎么跑见 §3.3）—— 这两条结论的前提都是
「今天恰好没有冲突数据」，而那种前提会过期。
...
两条都实现完并活体验证过了。这一节只写**三件上面那两节看不出来的事**：
**实际和方案哪里不一样**、**重跑出来的前提数字**、**验的时候撞到的坑**。
方案本身没有推翻的地方 —— 两节「别这么写」和三个坑全部照做。

### 3.1 需求一：和方案的四处差异

1. **回填 56 条，一条 NULL 都没剩**（方案里预计「18 条自由文本留 NULL」）。
   原因不是放宽了规则，是迁移里把那 18 个自由文本 `tool_name` **逐条写死**成
   `_LEGACY_AREA`（`zzz1area_cc_feedback_area.py`）。§1.5 坑 3 那条纪律没变 ——
   **匹配不上的仍然留 NULL**，只是这次没有匹配不上的。
   关键词匹配被否掉的理由写在那张表上面：`执行报告` 和 `接口场景执行器` 只差两个字，
   却分属 `report` / `api_run`（一个是报告读出来不对，一个是执行器本身跑错）。
2. **`AREAS` 放在 `app/models/cc_feedback.py`，是那 14 个值和**顺序**的唯一出处。**
   顺序不是字母序也不是条数序，是「离 CC 干活最近」—— 前端的列表顺序、下拉顺序、
   顶部计数块的 0 条尾巴全从它派生。加一个域只改这一处。
3. **`_TOOL_AREA` 只映射 49 个工具，剩下 14 个是故意不映射的**（`lum_*` 共 63 个）：
   mock/代理观测 5（`lum_llm_mock_*` 4 + `lum_proxy_capture`）、Skill 存取 3、
   失败归因 2（`lum_submit_analysis` / `lum_list_pending_confirm`）、
   纯定位 2（`lum_list_projects` / `lum_list_branches`）、反馈通道自己 2。
   **不映射 = 落 NULL = 交给 AI 分诊判**，比硬塞一个域好：这几个要么压根不属于
   某一块子系统（定位类），要么撞到它的时候坏的多半是别的地方。
   > 重算这个差集：`grep -oP '^\s+name="\Klum_[a-z_]+' backend/app/mcp/__init__.py`
   > 对 `_TOOL_AREA` 的键取差集。**新增工具不会报错**，只会静静地落 NULL。
4. **`report()` 软兜底，`triage()` 硬校验。** 同一个非法 `area` 两种下场是刻意的：
   上报侧填错一个不认识的值**不拒收**这条反馈（按兜底值落，返回里 `areaIgnored`
   说一声）—— 反馈通道最不该做的事就是因为一个分类字段把正文丢了；
   处置侧页面上是下拉，**非法值只可能来自脚本**，所以直接 `_err`。

顺带把 §1.6 那件白捡的做了：`_siblings_for()` 的候选集从「`tool_name` 精确相等」
放宽成 `or_(tool_name 相等, and_(同域, 同类))`。受益最大的正是那 8 条名字个个不同的
`AI 评审规则 xxx` —— 此前它们互相都不是判重候选。

**指纹一个字没动**（仍只有 `tool_name` + 归一化标题），封样测试盯着。

### 3.2 需求二：口径一句话

**生效范围 = 项目范围 ∩ Key 范围**，Key 只能更窄，决策点只有 `pick_scope()` 一个。
三条容易记反的：

- Key 的 `allowed_tools` 为 **NULL = 跟随项目**；**`[]` = 一个工具都不给**（不是不限制）。
- Key 选了项目范围外的工具 → **丢掉**，并在接口返回里报 `blockedByProject`
  （不是静默丢，也不是报错拒绝建 Key）。
- **工具范围不是权限边界**（§2.8 第 1 条），数据范围仍然只由 `project_id` 决定。

迁移 `zzz2kscope_key_scope_intersection.py` **无 DDL**，只跑 §2.6 那条 SQL：
非 0 就中止并把那几把 Key 打印出来，确认过了用 `LUMIERE_KEY_SCOPE_ACK=1` 放行。

### 3.3 重跑出来的前提数字（2026-09-03）

| 数字 | 2026-09-02 | 2026-09-03 重跑 | 怎么跑 |
|---|---|---|---|
| 遗留 Key 范围（§2.6 那条 SQL） | 0 | **0**（3 把活跃 Key 的 `allowed_tools` 全是 jsonb `null` 标量） | 见 §2.6 代码块 |
| 反馈总数 | 56 | 56 | `select count(*) from cc_feedback` |
| 注册工具数 | 63 | 63 | 见 §3.1 第 3 条那条 grep |

回填后的域分布（迁移跑完，dev 库）：
`ai_review` 15 / `sync` 12 / `case` 8 / `gate` 7 / `api_run` 6 / `report` 4 /
`note` 2 / `diff` 1 / `spec` 1 —— 加起来 56，**NULL 0 条**。
另外 5 个域今天 0 条（`apidoc` / `ui_script` / `qa_review` / `env` / `other`），
在页面上收成顶部那行灰字尾巴、不可点。

### 3.4 验的时候：两套测试 + 活体，和四个 antd v6 的坑

- `cd backend && .venv/bin/python -m pytest tests/ -q` → **2196 passed**。
  （`pytest backend/tests/` 从仓库根跑会 `ModuleNotFoundError` —— 必须 `cd backend`。）
- 根目录那套（真打接口）→ **628 passed**（502s），用独占库 `lumiere_test_area`。
  这两块新增的真接口测试共 59 条（`tests/api/mcp_keys/` + `tests/api/cc_feedback/`），
  都在这一趟里。
- 活体验证：需求二 **20 项**、需求一 **26 项**，都是 0 失败。
  用的是**本 worktree 的 vite 真页面** + `pg.route('**/api/**')` 造响应。
  **为什么这里用 mock**：最需要看的两个状态 dev 库产不出来 ——「待判域」（NULL）
  和「0 条的域」在真库里都不存在；而后端那半有真 HTTP 的 API 测试封着
  （`tests/api/cc_feedback/` + `tests/api/mcp_keys/`），不靠页面证。

四个 antd v6 的 DOM 事实（写脚本的时候一个个撞出来的，下次省点时间）：

1. `.ant-modal-content` **没了** → 用 `[role=dialog]`；`.ant-modal-container` 才是新类名。
2. `.ant-select-selection-placeholder` **改名**成 `.ant-select-placeholder`。
   选择器写错的表现和「产品坏了」一模一样 —— 得先 dump 一遍 DOM 才分得清。
3. **两个汉字之间 antd 会插一个空格**：按钮无障碍名是「保 存」「提 交」，
   所以要用 `re.compile(r'提\s*交')` 这类正则，写死字面量必然找不到。
4. **没有 `href` 的 `<a>` 在无障碍树里没有 `link` 角色**（顶部那排计数块就是
   `<a onClick>`），`get_by_role('link', ...)` 永远超时；按 DOM 顺序捞文本才行。
   另外 `innerText` 在标签和紧邻的计数 `<span>` 之间**不插空格**（那 5px 是
   `marginLeft`），期望值要写 `'AI 评审9'` 不是 `'AI 评审 9'`。
