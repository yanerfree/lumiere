...
def test_审核和模块列默认显示_来源和Flaky默认收起():
    """默认显示哪几列由使用者定，这里钉的是**当前裁定**（2026-08-24 用户要求）：
    默认显示 用例ID/标题/优先级/**模块**/状态/覆盖/**审核**/更新时间/操作；
    来源、Flaky、**类型**、**废弃审核** 默认收起。
    类型：库里几乎清一色「场景」，一列 20 多行同一个词 = 只占宽度不给信息。
    废弃审核：平时整列是「—」，**但有待废审时列表页会自动把它显示出来**
