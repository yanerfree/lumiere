...
⓿ 【每轮先问"该干什么"，别自己拍】lum_next_duty 给出待办队列（归因 / 复跑 /
   接口变动 / 补用例 / 补场景 / 自证 / 待废），每条带「下一步调哪个工具」。
   **这些队列已经是平台替你扫完挑出来的 —— 你的活是"写对"，查错交给平台那一轮。**
   所以**写完一批别自己逐条过门禁**：调 lum_review_batch 让平台批量扫（真跑六维 +
   断言咬合 + 顺带模块体检），扫出来的问题自动回到 next_duty，你回来只改被挑出来的；
   覆盖缺口、环境残留也在那一轮一起给，不用每条自己叫。写错的人看不出自己错，
   平台那一轮是**独立复查**，比你自查更防假绿，不是多此一举。
   单条卡住想当场看一眼，才用 lum_check_deliverable / lum_review_case；想单看一个模块
   还缺什么用 lum_module_checkup（都按需自查，不必每条）。
   新版本分支复用上一版用例，先 lum_check_branch 对账 —— 别照着旧清单硬抄。
   **这些工具怎么判、判据是什么，看它们各自的描述和返回值，这里不复述。**

① 生成「步骤用例」（功能用例）只有一条路：
   **先在被测系统里活体验证过，再用 lum_create_case 一条条回写成果。**
   ...
   **那不是测试，那只是把现状抄了一遍。**
   ...
   比对出来只有三种结果，各走各的路：
     ...
     · **不一致** → **这就是你要找的东西**。预期**按需求写**（不是按实测写），
       用例会红 —— 红得对。然后调 lum_submit_analysis 提 cause=product_defect，
...
①-0 【动手之前先查这个模块已经有什么】调 lum_list_cases 带上 module 筛选，
   ...
   ⚠ **判重只看 lum_list_cases，别拿接口场景当依据**。lum_list_api_tests 返回里
   分了两组：`boundToCases`（一个用例一条）和 `standalone`（**无主场景**）。
   `standalone` 现在**恒为空**，是个哨兵：**里面一旦有东西，说明有人绕过了约束，
   那不是谁的产物，别算进判重**（曾有 CC 看到孤儿场景全绿就不写新用例、改去"重绑"）。
...
①-1-B 【做的过程中发现场景不对，当场改，别攒着】
   ...
   发现下面这些，**直接改/直接补，不用等谁批准**（改完在回复里说一句改了什么）：
     ...
     · 标题/预期和实测下来的真实流程对不上 → 用 lum_update_case 改（你写错了自己改）
...
② 接口场景**只有一种**：你亲手活体验证过、绑在某条用例上的多步 E2E 链。
   回推走 lum_sync_orchestrated_scenario（登录→造→断言→清理），
   用 source_case_id 绑定某功能用例、**共享该用例的场景变量**。

   （「单接口·凭文档 AI 造」那种已下线 —— 不绑用例、场景变量挂不上，结构上跑不了。
     生成归你，平台只做呈现和回推通道；库里那几条老的无主场景判重时别算，见 ①-0。）

③ 活体验证后「回推同步」= 步骤用例 + 编排接口场景 + 场景变量。通道：
   lum_get_sync_spec（先对齐口径）→ lum_list_global_data（看可引用项）→
   lum_create_case（步骤用例）+ lum_upsert_scenario_variables（场景变量）+ lum_sync_orchestrated_scenario（接口场景）
   → lum_run_api_test（执行验证）。

   ③-0 【lum_get_sync_spec 一个会话只调一次，别每轮重调】它返回的规范正文
   （两万多字）**在一个会话里是不变的常量**。第一次回推前调一次、把 playbook 记在
   手边（或落到本地一个临时文件）反复用就行，后面每一条用例回推都别再调它 ——
   每轮重调等于把同一份两万字重新拉一遍，纯浪费。规范只会随平台版本更新，而平台
   一更新就会断开重连、你会重新读到这份说明和新规范，所以"会话内缓存"永远不会用到旧规范。
   要单独查某一节（改断言查 kind='timing'、写 UI 脚本查 kind='ui_script'）另说，那是按需取小节。

④ 变量纪律（回推时必须遵守，硬约束）：脚本里**不允许写死数据变量**。任何取值只能来自
   ①场景变量 ${名字}/${SV_名字}  ②项目级全局引用（${BASE_URL}/账号/token，见 lum_list_global_data）
   ③步骤间提取物（上一步 variables_extract）。lum_sync_orchestrated_scenario 会硬拦截悬空 ${x}。
   ...
   **造数据是你的活**，别指望环境里"刚好有"。按资源性质二选一：
   ...
   ▸ 路线 B【共享基础数据】：多条用例都要用、且反复重建代价大的底座
     （上游/负载、隔离上下文、长期存在的消费方应用……）。三步走：
       1. 先 lum_list_global_data 查项目里有没有登记过；
       2. **没有 → 你自己调接口把它造出来**（活体验证时就造，这是你的责任），
          造完**不要清理**，它要留给后续场景复用；
       3. 造好后（或本来就有）用 lum_upsert_automation_resource 登记 exists_check
...
⑤ 【默认先活体验证，别凭文档编】只要能连上被测系统（有可访问的环境地址和账号），
   ...
   再据此回推。不要读完接口文档就直接生成。
   · 平台**不再提供**"凭文档造"那条路（原 lum_generate_api_test 已下线）。连不上环境就
     先把环境弄通，或者只回推步骤用例（target_level=spec）把接口那一维明明白白欠着 ——
     欠着是事实，编一条跑不了的场景是假象。
   · 判断依据是"能不能连上"，不是"手上有没有文档"。有文档但环境也能连 → 仍然要活体验证。

⑥ 【一个用例 = 一条接口场景】lum_sync_orchestrated_scenario 按 source_case_id 幂等：
...
⑦ 【新增用例之前先报清单，等用户确认】
   ...
   **判据是「这个动作回滚贵不贵」，不是「是不是写库」**（2026-08-24 收窄）：
     ...
     要报清单等确认 —— 新建用例、删用例、改动**别人**的用例、
       往接口库里加节点（lum_create_case / lum_create_api_node / 删改类动作）
     不用报，直接做 —— 补一个还欠着的维度（回推脚本/接口场景/场景变量）、
       改**你自己**刚写错的字、改和实测不符的步骤（lum_update_case）。
       做完在回复里说一句改了什么就行。

   补一维、改个错字也要走一遍确认的话，人一天被问十几次，几次之后就闭眼说"都对"，
   **这道确认就废了** —— 留给真正贵的那几个动作。
...
读不到前端代码（不在同一个仓库、手上只有接口文档）**不是**编页面的理由 —— 编出来的
步骤没人跑得下去，标个"待确认"也没人回来确认，只会以"看着像写好了"的样子躺在库里。
按 ⑤ 办：能连上环境就在页面上点一遍把文案抄回来；连不上就只回推能确定的那部分
（target_level=spec），把欠的那一维明明白白欠着。

【UI 自动化脚本】平台不生成脚本。你在自己机器上用 Playwright 写、本地真跑通：
先 lum_get_sync_spec(kind='ui_script') 对齐写法（变量必须走 os.getenv 顶格声明，
平台执行时把所选环境的真值替换进默认值）→ lum_sync_ui_script 回推（入库前硬拦
写死的地址和凭据）→ lum_run_ui_script(case_id, env_id) 让**平台**在标准环境上
再跑一遍。你本机跑通不算数 —— 本机有 dev server、有残留数据、有 cookie。

【质量规范不用背，入库时会逐条告诉你】module 为空 / preconditions 太短 /
步骤写成接口调用风格 / 预期里有模糊词 / 同模块标题重复 —— lum_create_case
...
具体怎么推、怎么取、参数含义，看 lum_push_skill / lum_list_skills / lum_pull_skill
各自的描述 —— 这里只补两条那里不讲的：① 推之前把清单报给用户确认（同 ⑦ 条纪律）；
② 本通道只存**客户端侧**执行的 skill（跑在你机器上，用 Bash/Edit/Playwright），
内置 lum-* 是平台侧执行的另一类，别用 lum_push_skill 覆盖它们。
""",
)
...
_register(
    duty.next_duty,
    name="lum_next_duty",
    description="【每轮上来先问这个】这一轮该干什么：一次给全部待办队列，下面这几类最常见 —— ①待归因（红了还没分析的失败，带现象/红了几次/第几次复发）②待复跑（已确认修好的，跑绿了跟进单自动关）③**待处理接口变动**（版本升级分支对账命中的，一条条改预期——**按新版本的需求写，不是打开新版本跑一遍照着抄**，那是把实现抄了一遍、把新引入的 bug 固化成预期）④**待补用例**（新版本的新端点，谁都没覆盖它，不补就零覆盖且**永远不会报错**）⑤待补场景（审核反复提到的模块级缺口，被提到越多越该补）⑥待自证（回推四问没答的）⑦**等人拍板的废弃**（探不出来落到人手里的，别绕过它自己废）；另有待补 testid（去被测前端仓提 MR）、回来写 UI（抓手有了、还欠 UI 脚本）、平台反馈有回音（报给平台的问题有结论了），各自含义看返回里的 usage。每条都带「下一步该调哪个工具」，按堵得最死的排。**别自己关跟进单** —— 跑绿平台自动关；要强行放过得人工关闭并写原因。参数: branch_id(分支UUID), limit(每队列条数，默认10)",
)

_register(
    review.review_case,
    name="lum_review_case",
    description="【单条深审·别等人；一批用例走 lum_review_batch】按六维评审一条用例并落库审核结论：场景合理性 / 验证点到位 / 接口必要性 / UI 脚本正确性 / **本条覆盖完整性** / 可执行与纪律（不适用的维度自动摊掉权重）。⚠「本条覆盖完整性」只判**这一条自己承诺的东西验全了没有**（标题和预期里说到的每件事，断言里是不是都验了）——模块还缺哪些场景走 coverageGaps，是情报，**不扣这一条的分**，别为了提分去补邻居用例。**判定不由 AI 说，也不看分数**：有 blocker 一律不过、**两处以上 major 不过**、没真跑成功 → 第三种结论 `inconclusive`（无法审核：既不算通过也不算打回，环境弄好再审）。**分数只做体检和排序，不参与判定**（六维加权是模型给的，同一条两次拿到 86 和 78 是常事，拿抖动的数当闸门没法照着改）。blocker = 放进回归就是假绿或根本跑不了（断言恒真、只断控制面状态就当生效、预期照着实现抄、UI 脚本必挂）。返回 mustFix 逐条指到步骤名/断言/脚本位置。**run_first 默认 true**：先真跑一遍再评（UI 优先，debug 模式不进通过率）——断言咬不咬得住静态看不出来，「接口场景验的端点页面根本不调」更是只有真跑才暴露。**不传 env_id 也没关系**，平台按**本项目**的历史执行环境/默认环境自己挑（跨项目挑错环境的 bug 2026-09-01 已修），本项目一个带 BASE_URL 的环境都没有才跳过真跑。显式传 run_first=false 才是静态审，**结论强度差一个量级**（实测同一条：静态 84 分通过、真跑 56 分打回），reviewMode 会告诉你这次是哪种 —— 只在确实没有可用环境时才这么用。另有两条岔路，返回形状不同：这条正申请废弃时它改审「**该不该废**」；版本升级里没被对账清单命中、内容与上一版逐字一致、上一版已审通过的「照抄堆」直接四条件自动过审（decidedBy=system），都不走六维。⚠ 这是一次不间断的同步调用，run_first=true 时可能跑到分钟级，中途没有心跳——如果这次调用**超时/无响应就中止了，不代表没跑完**：评审是跑完就落库，**别当场重新审一遍**，调 lum_review_check 看是还在跑还是已经出结论了。真的在跑的话这个工具也会挡下来（返回 status=in_progress，不会重复触发一轮真跑）。参数: case_id(用例UUID), run_first(可选，**默认 true**), env_id(试跑用的环境UUID，不传平台按本项目自己挑)",
)

_register(
    review.review_status,
    name="lum_review_check",
    description="【lum_review_case 超时了先调这个，别重审】只读地查一条用例的审核状态，**不触发评审、不占环境、不跑任何东西**，随时可以调。三种回答：①status=in_progress —— 有一次审核正在跑（含别人在页面上发起的模块批量），带已经跑了多久，等一会再来查，这期间调 lum_review_case 只会被挡回来；②status=reviewed —— 已经有结论，返回跟 lum_review_case 同一形状的摘要（verdict/mustFix/niceToFix/summary/runAttribution），另带 stale=true 表示这条结论出具之后场景或 UI 脚本又被改过、结论可能对不上现在的内容；③status=not_reviewed —— 从没审过，去调 lum_review_case。参数: case_id(用例UUID)",
)

_register(
    review.review_batch,
    name="lum_review_batch",
    description="【写完一批就送这·平台独立扫一轮】写完一批用例别逐条自查，整批送进平台的**审核队列**扫一遍 —— 真跑六维 + 断言咬合 + 顺带模块体检，是**独立复查**（写错的人自己看不出错，所以这一轮比你自查更防假绿），挑出来的问题自动回到 lum_next_duty，你回来只改被挑出来的。这就是主路，别再每条自己过门禁。⚠ 送批时**不要自己循环调 lum_review_case** —— 那个是直调、一条也不排队，你并发送 20 条就是 20 次真跑同时打一个环境，而这条队列要防的两件事一件都吃不到：①**同环境串行**（两条脚本共用租户/账号，A 跑到一半 B 把 A 要用的数据删了 → A 莫名报错 → 审核判 A「脚本有问题」，**这是假打回**）②**熔断**（环境一挂，连续 3 条环境类失败就暂停整批；逐条调的话 20 条全标「无法审核」，看着像用例集体坏了）。**这批一定是真跑**，所以没有 run_first 参数。范围三选一：case_ids(逗号分隔，1 条=单条 / 多条=抽审) / module(模块名，连子模块一起，=模块全量) / 都不传=整个分支；scope='incremental' 只审没审过的和被打回的（**无法审核的也算没审过**）。单批上限 30 条，超了只排前 30。**人工发起的排在你前面**，入队后用 lum_review_batch_status 轮询，别重复入队。同一条已在这个环境的活跃批次里排着 → 自动合并不跑两遍。参数: branch_id(分支UUID), case_ids(可选), module(可选), env_id(可选，不传挑有 BASE_URL 的默认环境), scope(all/incremental，默认all), with_checkup(模块级审核时顺带做模块体检，默认true)",
)

_register(
    review.review_batch_status,
    name="lum_review_batch_status",
    description="【看审核批次跑到哪了】轮询一个批次的进度：每条的结论(approved/rejected/inconclusive)、当前在审哪条、通过/打回/无法审核各几条。**判完没完看 finished 字段**，别拿 done==total 猜（total 为 0 或中途熔断时那个猜法不成立）。**status=paused 就是熔断了** —— 连续 3 条环境类失败，那不是用例的问题，去把环境弄好再在页面上续跑，接着刷只会继续红。参数: batch_id(批次UUID，lum_review_batch 返回的)",
)

_register(
    review.module_checkup,
    name="lum_module_checkup",
...
_register(
    ...
    name="lum_check_deliverable",
    description="【交付门禁·单条按需自查】默认不用每条都跑 —— 写完一批交给 lum_review_batch 让平台扫一轮就够了（那一轮真跑六维+咬合，是独立复查）。这个留给你**单条想当场确认**时用：只读不改任何状态，回三类结论：blockers=交不了（有一条就是不可交付：欠维度/一步没跑过/有步骤挂着/断言把布尔写成字符串这种必然假红）、risks=交得了但脆（典型是异步断言没开 retry_timeout_ms —— 跑绿了也是侥幸跑赢时间窗，换台机器就红；还有只跑了勾选的一部分、流量被截断）、notes=要你自己判断的（疑似越界的测试点、只用 body_contains 兑付「应产生/应记入」这类承诺、请求体里的驼峰键）。参数: case_id(用例UUID)",
)
...
_register(
    ...
    name="lum_list_branch_endpoints",
...
_register(
    ...
    name="lum_llm_mock_requests",
    description="**网关到底往上游发了什么** —— 断言用的。鉴权头有没有正确注入、模型名有没有按映射改写、参数有没有被篡改，这些在网关下游根本看不见（客户端只能看到最终响应），这是唯一的观测点。智能应答路由还返回 smartMeta：mode(命中哪条指令)、**stream(网关实际发出的值，流式降级有没有真发生只能看它)**、hasStreamOptions、loopStage，checker 角色再带 checkedLen/envelopeLen/verdict(护栏到底拿到了多长的正文)。断言前先 lum_llm_mock_reset 清一次。参数: path(可选), limit(默认20)",
)

_register(
    mocks.llm_mock_reset,
    name="lum_llm_mock_reset",
    description="清掉上游请求记录。**断言「上游收到几次」之前必须先清** —— 不清的话上一轮的记录还在，「只应收到 1 次」会假过，而假过比假红更难发现。参数: path(可选，不传清全部)",
)

_register(
    mocks.proxy_capture,
    name="lum_proxy_capture",
...
_register(
    ...
    description="改一条已有用例。只传要改的字段。**你写错了自己改，别喊人**（标题打错字、步骤和实测不符都用这个）。"
                ...
                "待办来源：lum_list_cases(bug_state='blocked') 跟 git 上已关闭 issue 取交集。, "
...
_register(
    ...
    name="lum_create_api_node",
    description="往**接口库**里加一个接口或文件夹。这是在维护接口文档，不产生可执行的测试——要可执行的用 lum_sync_orchestrated_scenario。参数: project_id(项目UUID), name(名称), node_type(endpoint/folder,默认endpoint), method(GET/POST/PUT/DELETE等), url(接口路径), parent_id(可选,父文件夹UUID), params(可选,查询参数[{key,value,desc}]), headers(可选,[{key,value,desc}]), body(可选,请求体), body_type(可选,json/form/raw/none), auth(可选,{type,token}), description(可选), sort_order(排序,默认0)",
)
...
_register(
    ...
    description="列出**本项目**的测试环境（id + 名称）。环境是项目级的，一个项目的环境在别的项目里看不到、也用不了。拿到 env_id 再去 lum_get_merged_variables 看它有哪些变量可引用。参数: project_id(项目UUID)",
)
...
_register(
    ...
    name="lum_submit_analysis",
    description="【失败归因】把你对某次失败的判断写回平台。先 lum_get_ui_script_result 拿证据包和 run_id，"
...
_register(
    plans.list_plans,
    name="lum_list_plans",
    description="【执行报告】列出项目下的测试计划，拿 planId。拿到 planId 就能调 lum_get_report_summary / lum_get_failed_scenarios（只有 reportId 也行，它们会自己反查计划）。返回含用例数、最近一次 reportId。参数: project_id(项目UUID), status(可选: draft/executing/completed), limit(默认20)",
)

_register(
    plans.create_plan,
    name="lum_create_plan",
    description="【执行报告】新建一个自动化测试计划（只建不跑，触发要另调 lum_run_plan）。参数: project_id, branch_id, name(计划名), case_ids(逗号分隔的用例UUID), test_type(e2e跑UI脚本/api跑接口脚本，默认e2e), environment_id(强烈建议传，不传执行时拿不到 BASE_URL 和账号), retry_count(默认0)",
)

_register(
    plans.run_plan,
    name="lum_run_plan",
    description="【执行报告】触发计划在**平台执行器**上跑（这一跑算回归、进通过率口径）。立刻返回 taskId 和 reportId，执行是异步的 —— 拿 reportId 轮询 lum_get_report_summary。注意：你只是按了按钮，结果由平台执行器写；你不能写执行结果、也不能改用例通过状态。参数: plan_id(计划UUID)",
)

_register(
    plans.list_reports,
    name="lum_list_reports",
    description="【执行报告】列出测试报告，拿 reportId + 通过率。参数: project_id(项目UUID), plan_id(可选，按计划过滤), limit(默认20)",
)

_register(
    test_reports.get_report_summary,
    name="lum_get_report_summary",
    description="一次执行的总览：通过 / 失败 / 跳过 / 通过率，以及按模块的分布。参数: plan_id 和 report_id **给一个就行**（只有 lum_run_plan 返回的 reportId 也能查，会自己反查计划）",
)

_register(
    test_reports.get_failed_scenarios,
    name="lum_get_failed_scenarios",
    description="【执行报告】拿这次报告里所有失败的用例，**每条带 runId** —— 用它调 lum_get_ui_script_result 看证据包（截图路径 / 流量 / 平台的现象初判），判完再调 lum_submit_analysis 回填归因。参数: plan_id 和 report_id **给一个就行**（不传 report_id 取该计划最近一次）",
)
...
# lum_generate_api_test（凭接口文档让平台 AI 造单接口场景）2026-08-15 随
...
_register(
    ...
    name="lum_get_api_test",
    description="读一条接口场景的全部内容：每一步的请求、断言、提取了什么变量。想知道它到底怎么测的就读这个。**只想改几个断言就别整条读** —— 实测最大的场景 59 步 53,887 B，而按字段拆开没有一处冗余（步骤名 24%、断言 23%，全是正文），省不出来只能少读：先 outline=True 拿准确步骤名（不含断言/头/体），再 step_names 点名读那几步（字段照旧给全），改完用 lum_sync_orchestrated_scenario(mode='patch') 推回去。⚠ 这两种都是**部分读取**，返回里带 partial/stepsTotal 和警告：拿着部分结果走默认的 mode='replace' 会把没读到的步骤整个删掉，而且当场不报错。参数: scenario_id(场景UUID), step_names(可选，**点名首选序号**——outline 给的 sortOrder，如 '12,13,27'；名字也收，但库里 5% 的步骤名自带半角逗号，那种必须传 JSON 数组 '[\"名字A\",\"名字B\"]' 否则被切坏), outline(可选，默认false)",
)

_register(
    api_tests.check_env_hygiene,
    name="lum_check_env_hygiene",
...
_register(
    ...
    name="lum_check_assertion_bite",
    description="【验断言到底有没有用】把**改状态的那个动作步**跳掉跑一遍：后面的断言该红就是有效，照样绿就是恒真（动作前后都成立，动作坏了也不会红）。绿的用例≠有效的用例——方向写反的断言也是绿的，断言条数和强度指纹都判不了这件事，只有「删掉原因、看结果是否消失」能判。跳的必须是动作步（审批通过/禁用服务/驳回/删除），别跳产出 id 的创建步（后面全卡在变量未解析，什么都证明不了）。只读：不写步骤状态、不建报告、不动用例维度；但**请求是真发的**（没被跳掉的步骤照跑）。⚠ 跳的就是清理步时，那一趟造的数据不会被删 —— 残留归你自己收（lum_check_env_hygiene 看不见它，变异运行不留痕）。参数: case_id(用例UUID), skip_steps(要跳掉的步骤，**首选序号**（lum_get_api_test 的 outline=True 给的 sortOrder，如 '7' 或 '7,9'）；给名字要逐字一致，名字里带半角逗号的传 JSON 数组 '[\"名字A\"]'), env_id(强烈建议，不传没有 BASE_URL/账号链子跑不起来)",
)

_register(
    api_tests.run_api_test,
    name="lum_run_api_test",
...
# ── 需求→用例流水线：已下线 ────────────────────────
#
# `lum_create_scenario_task` / `lum_confirm_and_generate` / `lum_get_scenario_task` /
# `lum_query_coverage_matrix` / `lum_get_generation_stats` 五个工具已摘除，
...
_register(
    ...
    name="lum_get_ui_script_result",
    description="【失败证据包】拿这条用例最近一次执行的**完整证据**，用来判断为什么挂：截图（返回**文件路径**，和平台同机，直接 Read 打开看图）、网络流量摘要（按状态码分桶 + 展开非 2xx 和写操作那几条，其余页面自身的 GET 已折叠）、stdout 尾部、run_id，以及平台按确定性规则给的**现象**初判 failure_phenomenon（timeout / element_not_found / assertion_mismatch / http_5xx / script_error / dependency_unresolved / unknown）。**UI 和接口场景都收**（返回里的 script_type 说明是哪一维）——接口执行没有截图和浏览器流量，但 error_summary / stdout 轨迹 / 现象都在，够写 evidence。⚠ **要归因就把 run_id 传上**（lum_get_failed_scenarios 给的那个）：不传只取「最近一次」，而这条用例可能已经被复跑过 —— 活体撞到过 TC-DYGL-00013 六次接口执行、最近一次是 passed，于是证据包里什么指针都写不出来，而 lum_submit_analysis 又拒收 passed 的执行。**报告指着失败那次、这里给最新那次**，不传 run_id 就会踩这个错位。⚠ 现象不是归因 —— 平台判「是什么」，「为什么」由你判断。参数: case_id(用例UUID), run_id(可选但归因时强烈建议), script_type(可选 ui/api，不传取最近一次)",
)
...
_register(
    skills.push_skill,
    name="lum_push_skill",
    description="【把本项目的 skill 推上平台】读你本地 .claude/skills/<name>/ 的内容，推到 Lumiere 存起来，默认 visibility=public 即其它项目可取用。存的是**客户端侧执行**的 skill（跑在开发者机器的 Claude Code 里，用 Bash/Edit/Playwright），平台只存取、永不当 prompt 执行 —— 跟内置 lum-* 不是一类。同名会覆盖，覆盖前自动留档可回滚。参数: project_id(项目UUID), content(SKILL.md全文,必填), name(可选,不传则取 frontmatter 里的 name), files(可选,附属文件{相对路径:文本内容},如 references/api.md), description(可选,不传取 frontmatter), visibility(public全平台可取/project仅本项目,默认public), overwrite(默认true)",
)

_register(
    skills.list_skills,
    name="lum_list_skills",
    description="【看平台上有哪些 skill 可取用】列出可用的项目 skill(客户端侧执行那类)。传 project_id = 本项目的 + 全平台共享的；不传 = 只看全平台共享的。返回里的 skillId 拿去喂 lum_pull_skill。参数: project_id(可选,项目UUID), include_shared(默认true)",
)
