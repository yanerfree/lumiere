...
①-A 【预期从哪来 —— 这一条最要紧，看漏了整批用例都白写】
   ...
   预期的来源按这个顺序找，**找到一个就停**：
     1. **需求文档 / 设计文档 / 接口契约**（PRD、README、docs/、OpenAPI 描述、
        代码里的业务规则注释）—— 你是 Claude Code，仓库就在手边，直接读。
        **这是最该用也最容易被跳过的一步。**
     2. 跟用户在对话里确认
     3. 都没有 → 才退回"以实测行为为准"，并且必须在
        expected_confirmed_note 里写明**「无文档依据，按实测行为记录」**，
        让后面的人知道这条预期的可信度。

   实测和需求对不上的时候，那是**缺陷线索**，不是"预期要改成实测值"。
   照实测写，就等于替被测系统把 bug 洗白了。
...
_register(
    ...
    name="tb_create_case",
    description="新建一条用例（手工步骤）。用例是「测什么」的载体——接口场景和 UI 脚本都挂在它下面，所以先有用例再有脚本。编号和目录自动生成。**入库要过门禁**：标题完全同名硬拒、标题含模糊词（操作成功/显示正常/无报错/符合预期）硬拒。标题相似只提醒不拦。**P0 三件套不拦你**：同源生成的三份产物容易互相一致而不正确（典型是把「创建成功」做成「返回 200」），所以预期必须有**外部锚点**：先读需求/设计文档（你是 Claude Code，仓库就在手边，这一步最该做也最容易被跳过），文档说不清再在对话里跟用户确认，都没有才退回实测行为并注明「无文档依据」。把依据用 expected_confirmed_by / expected_confirmed_note 带上来（落款写清是文档还是人，例：「docs/订阅.md §3.2」/「用户（对话确认）」）—— 平台只记录、不拦截，没带只回一句提醒。⚠ **别把实测结果直接当预期**：系统有 bug 时你会把 bug 写成「预期」，而三份产物同源，会一起错还全绿。参数: branch_id, title, module(中文如'服务管理'), case_type(**这条在测什么形态的东西，跟做不做 UI 无关**：e2e=场景（验证一个完整功能，多步编排）/ api=单接口（针对单个接口的参数、边界、越权）；做几维看 target_level), priority(P0-P3), preconditions(前置条件), steps([{seq,action,expected}]), expected_result, target_level(这条要做到什么程度: spec只要步骤/spec_api步骤+接口/full三件套，默认spec), **target_level_reason(不做某一维就说一句为什么——只有 target_level 一个值时，人分不出你是判断过不需要、还是没想就用了默认值；不写只提醒不拦)**, expected_confirmed_by(跟谁确认的), expected_confirmed_note(确认了什么，把对话里那句原话带上来)",
)
