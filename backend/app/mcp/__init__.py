...
当用户要求生成测试用例时，按下面这份操作清单执行 ——
**它是上面那些规则的落地步骤，不是另一套流程**。有冲突一律以上面为准
（尤其：预期从哪来看 ①-A、必须活体验证看 ⑤、动库前报清单看 ⑦）。
...
第二步：了解真实页面（最关键，不能跳过）
- **这一步取的是"页面上那些字长什么样"的素材，不是用来定预期的**，
  也**不替代活体验证**（见 ⑤）：按钮文案、字段标签、Toast 原文抄代码最准，
  但"这个功能应该是什么行为"只能按 ①-A 三步走（读需求 / 读实现+实测 / 自己比对）。
...
- 找不到前端代码时**不要凭 API 定义推断页面结构**（那就是"凭文档编"，见 ⑤）：
  能连上被测系统就去页面上真走一遍把文案抄回来；连不上就只回推步骤用例
  （target_level=spec）把 UI 那一维明明白白欠着 —— 欠着是事实，编一条是假象。

第三步：检查去重
- 调用 tb_list_cases 检查同模块已有用例，避免重复

第四步：生成用例
- 基于第二步获取的真实 UI 信息，生成用例步骤（**预期按 ①-A 写，不是照实测抄**）
...
- 每条用例只验证一个测试点
- P0 占比不超过 15%
- case_type **看测试对象**，别一律填 e2e：api=某一个接口的参数/权限，
  e2e=场景（某功能是否按需实现）。跟做不做 UI、步骤多少、造数用了几个接口都无关
...
_register(
    ...
    description="【失败证据包】拿这条用例最近一次执行的**完整证据**，用来判断为什么挂：截图（返回**文件路径**，和平台同机，直接 Read 打开看图）、网络流量摘要（按状态码分桶 + 展开非 2xx 和写操作那几条，其余页面自身的 GET 已折叠）、stdout 尾部、run_id，以及平台按确定性规则给的**现象**初判 failure_phenomenon（timeout / element_not_found / assertion_mismatch / http_5xx / script_error / dependency_unresolved / unknown）。**UI 和接口场景都收**（不传 script_type 就是最近那一次，返回里的 script_type 说明是哪一维）——接口执行没有截图和浏览器流量，但 error_summary / stdout 轨迹 / 现象都在，够写 evidence。⚠ 现象不是归因 —— 平台判「是什么」，「为什么」由你判断。⚠ **入参是 case_id，不是 runId**（runId 是下一步 tb_submit_analysis 用的）。参数: case_id(用例UUID), script_type(可选 ui/api，不传取最近一次)",
)
