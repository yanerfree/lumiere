...
async def next_duty(session: AsyncSession, branch_id: str, limit: int = 10) -> dict:
    ...
    return {
        ...
        # usage 精简：每条队列项自带「下一步」（该调哪个工具），这里不再把同一份指引
        # 用大段散文复述一遍 —— 那段每次调用都回、纯冗余。只留两条最容易踩的铁律。
        "usage": "按「建议顺序」做；每条自带「下一步」说清该调哪个工具。"
                 "两条铁律：① 待处理接口变动，**预期按新版本的需求写，不是按新版本的实测抄**；"
                 "② **别自己关单** —— 跑绿了平台自动关，强行放过要人工关闭并写原因。",
    }
