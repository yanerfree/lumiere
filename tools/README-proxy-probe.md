...
| | 用途 | 在哪 |
|---|---|---|
| **Lumiere 页面**（主交付物） | 测试人员日常用。全程在浏览器里完成判断，不碰命令行 | 菜单 **测试工具 → 代理观测** |
| **独立单文件脚本** | 丢到任意测试机上跑（尤其是没装 Lumiere 的机器）、命令行验收 ①~⑥ | [tools/proxy_probe.py](proxy_probe.py) |
...
2. 复制**代理地址**（形如 `http://192.168.51.108:28900`），填到被测系统某个上游服务商的
   ...
   > 因为如果你从 `localhost` 打开 Lumiere，按地址栏拼出来会是 `http://127.0.0.1:28900`，
...
抽屉里**不是一锅端**，而是按链路分成标签页 —— 顶部先画出这条链路：
`客户端 <ip:port>  ──▶  代理 :28900  ──▶  上游 <host:port>`。

| 标签页 | 内容 |
|---|---|
| ① 客户端 ⇆ 代理 | **别人给代理的**：客户端发来的请求（原样，含 absolute-URI 请求行和 `Proxy-*` 头）、请求体；以及**代理回给客户端的应答**，并注明这个应答是代理自己生成的还是上游原样透传的 |
| ② 代理 ⇆ 上游 | **代理转发出去的**：改写后的请求（`origin-form`，已剥逐跳头，并列出剥掉了哪些）；上游回给代理的响应头 + 响应体 |
| ③ 隧道内数据 | 只有 CONNECT 有。上行/下行分别判定类型并给出十六进制原文 |

对比 ① 和 ② 就能自己确认：请求行有没有从 absolute-URI 改写成 origin-form
（不改，规范上游会回 400）；逐跳头有没有剥掉。

**CONNECT 的方向不会标错**：`HTTP/1.1 200 Connection Established` 是**代理自己**回给
客户端的（在 ① 里），不是上游发的；② 里明确写出「CONNECT 只建 TCP 连接，
没有向上游发送任何 HTTP 请求头」，上游响应那一栏也直接说明这一跳没有 HTTP 响应。

**隧道到底加密没加密是确定判定，不是含糊其辞**：TLS record 的首字节固定是
`0x16`（handshake）+ 版本 `0x03xx`，据此给出结论并把原始字节的十六进制列出来供核对：

- 打 TLS 端口 → 「确认是 TLS 加密流量：首字节 0x16 = TLS handshake，版本字段 0x0303（TLS 1.2）」，
  标签页标题直接写「隧道内数据（TLS 加密）」。
- 打明文端口 → 判定为明文 HTTP，**直接把隧道里的 `GET /v1/models` 显示出来**。
- 都不是 → 说明「不是 TLS（首字节 0x??，TLS 握手应为 0x16），也不像文本」，附十六进制。

两点实现约束：

- **预览是「旁抄」不是缓冲**：数据收到就立刻转出去，只额外拷前 4KB 留证。
  所以 SSE 仍然是边到边流的（实测事件到达时刻 `[0.0, 0.4, 0.8, 1.2, 1.6]`，不是攒齐才给）。
- **报文原样显示，不做任何删改**：含 `Proxy-Authorization` 的完整值，另外把 base64
  解码出的用户名/密码列出来（base64 肉眼看不出内容），可一键复制 `user:pass`。
  这是测试辅助工具，职责是如实呈现收到了什么。
...
`GET /api/proxy-probe/records`（注意 Lumiere 全局中间件会把响应 key 转成
