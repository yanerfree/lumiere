...
- **密码在入库前就打掉了**：`Proxy-Authorization` 的值存成
  `Basic ****（密码已隐去，解析出 user=svc）`，明细里存的就是打码后的内容，不是原文。
...
**密码永远不会出现在日志里**（只记 `user=`），这是硬性要求，已在验收 ③ 中验证。
...
| ③ | 认证凭证不泄露 | **通过** — 日志出现 `user=myuser`；`grep -c mypassword` = **0** |
...
| ⑩ | 连发 CONNECT 和 GET → 两种形态清晰区分 | **通过** — CONNECT 1 行、GET 2 行；标签渲染色实测不同（`rgb(114,46,209)` vs `rgb(56,158,13)`）；认证列显示 `user=svc`，**密码 `secret123` 在表格和整页 HTML 中均不出现** |
| 附加 | 页面故障注入开关实时生效 | **通过** — 点「拒绝所有请求」后后端 `rejectAll=true`，此时 curl 返回码 `000`（连接被断开） |
| 附加 | 明细抽屉（原始请求 / 转发请求 / 上游响应） | **通过** — 三段齐全；原始是 `GET http://…`，转发是 `GET /v1/models HTTP/1.1`；剥掉的逐跳头列出 `Proxy-Authorization`、`Proxy-Connection`；响应头 `HTTP/1.1 200 OK` 和响应体预览可见；**抽屉里不出现密码 `secret123`**；加了旁抄后 SSE 仍是 `[0.0, 0.4, 0.8, 1.2, 1.6]` 逐个到达 |
...
python3 tools/proxy_probe_test_detail.py    # 明细抽屉三段 + 密码不泄露
```
...
上述所有故障注入日志中，密码 secret123、WRONGPASS 出现次数均为 0               通过
