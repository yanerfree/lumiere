...
| **QA 域评审升级：真跑页面对流量**（现行需求在 **§12~§15** —— 探索测试是**真的去操作**（建→改→删）；业务链路（保存完干嘛、谁审批）**从对方的场景和脚本里读**；每步要读业务逻辑（规则/状态/还能点什么），**业务不只增删改查**；**广度必须满、深度可打折**，`unreached`≠`seen_not_run`；两条被否掉的省事写法：「倒推页面操作」、「按域手写脚本」，正文已删只留理由） | [docs/qa-domain-live-verification-plan.md](docs/qa-domain-live-verification-plan.md) |
