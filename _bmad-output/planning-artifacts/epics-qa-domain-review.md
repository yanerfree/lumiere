...
- **S0.1 ✅ 已完成（2026-08-28）· 两个数都出来了，而且当前 `max_tokens` 明确不够。**
  ...
  **结论三（新发现·要写进 Epic 2 的 AC）：`arq` 的 `job_timeout = 600` 是 worker 全局的，
  已经在射程内。** 实测单批最慢 404s；`BATCH_CONCURRENCY = 3` 时 6 批要跑两波，
  两波之和轻易过 600s（本次并发 6 的全域墙钟就是 367s / 404s 两轮）。
  Epic 2 把 `max_tokens` 从 2400 提到 10000 之后**输出变长、单批更慢**，
  这个 600 会先撞上 —— 表现是整个域的评审直接没了，不是慢。
